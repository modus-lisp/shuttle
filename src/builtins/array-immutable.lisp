  ...
  (let ((ap (realm-array-proto realm)))
    (flet ((len (this)
      ...
      (def-method realm ap "toReversed" 0 (this args)
        (declare (ignore args))
        (let* ((o (to-object this)) (l (len o)) (out '()))
          (check-len l)
          ;; Spec reads from index (l-1) down to 0.
          (loop for i from (1- l) downto 0
                do (push (js-get o (princ-to-string i)) out))
          (make-array-object (nreverse out))))

      (def-method realm ap "with" 2 (this args)
        (let* ((o (to-object this))
          ...
          (check-len l)
          (let ((idx (truncate actual)) (out '()))
            ;; Spec: for the replaced index use VALUE (do NOT Get it).
            (dotimes (i l)
              (push (if (= i idx) value (js-get o (princ-to-string i))) out))
            (make-array-object (nreverse out)))))
      ...
      (def-method realm ap "toSpliced" 2 (this args)
        (let* ((this (to-object this))
          ...
          ;; newLen > 2^53-1 -> TypeError; then ArrayCreate: > 2^32-1 -> RangeError.
          (when (> new-len 9007199254740991)
            (js-throw (make-native-error "TypeError" "toSpliced result exceeds maximum array length")))
          (check-len new-len)
          ;; Read only the retained indices (spec skips the spliced-out range).
          (let ((out '()))
            (dotimes (i start) (push (js-get this (princ-to-string i)) out))
            (dolist (item insert) (push item out))
            (loop for i from (+ start skip) below l
