  ...
  (macrolet ((need-obj (v) `(unless (js-object-p ,v) (js-throw (make-native-error "TypeError" "not an object")))))
    ...
    (def-method realm octor "assign" 2 (this args)
      (let ((target (to-object (arg 0 args))))
        ...
        target))
    ;; freeze/seal go through SetIntegrityLevel, not the raw property table.  Mutating the table
    ;; skipped every host trap and could not fail, so `Object.freeze` silently did nothing to an
    ;; object that should have refused -- and never threw, which the spec requires when the
    ;; integrity level cannot be reached.
    (def-method realm octor "freeze" 1 (this args)
      (let ((o (arg 0 args)))
        (when (js-object-p o)
          (unless (set-integrity-level o :frozen)
            (js-throw (make-native-error "TypeError" "Cannot freeze"))))
        o))
    (def-method realm octor "seal" 1 (this args)
      (let ((o (arg 0 args)))
        (when (js-object-p o)
          (unless (set-integrity-level o :sealed)
            (js-throw (make-native-error "TypeError" "Cannot seal"))))
        o))
    ...
    (def-method realm octor "isFrozen" 1 (this args)
      (let ((o (arg 0 args)))
        (if (js-object-p o) (js-bool (test-integrity-level o :frozen)) *true*)))
    (def-method realm octor "isSealed" 1 (this args)
      (let ((o (arg 0 args)))
        (if (js-object-p o) (js-bool (test-integrity-level o :sealed)) *true*)))
