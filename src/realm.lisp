  ...
  (macrolet ((need-obj (v) `(unless (js-object-p ,v) (js-throw (make-native-error "TypeError" "not an object")))))
    ...
    (def-method realm octor "freeze" 1 (this args)
      (let ((o (arg 0 args)))
        (when (js-object-p o)
          ...
          (map-props o (lambda (k p) (declare (ignore k))
    ...
    (def-method realm octor "seal" 1 (this args)
      (let ((o (arg 0 args)))
        (when (js-object-p o)
          (setf (js-object-extensible o) nil)
          (map-props o (lambda (k p) (declare (ignore k)) (setf (prop-configurable p) nil))))
        o))
      ...
      (let ((o (arg 0 args)))
        (if (js-object-p o)
            (js-bool (and (not (js-extensible-p o))
                          (block frozen
                            (map-props o (lambda (k p) (declare (ignore k))
                                           (unless (and (not (prop-configurable p))
                                                        (or (prop-accessor p) (not (prop-writable p))))
                                             (return-from frozen nil))))
                            t)))
            *true*)))
      ...
      (let ((o (arg 0 args)))
        (if (js-object-p o)
            (js-bool (and (not (js-extensible-p o))
                          (block sealed
                            (map-props o (lambda (k p) (declare (ignore k))
                                           (unless (not (prop-configurable p))
                                             (return-from sealed nil))))
                            t)))
            *true*)))
